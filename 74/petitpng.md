# JSで ファイル生成 PNG編

22 McbeEringi

この記事は<https://qiita.com/McbeEringi/items/9928a9bc05798924e68c>の詳細版です。

---

## 動機

第三者の要望を受けてフォントファイルをビットマップに変換するアプリglyph_gen(<https://mcbeeringi.github.io/apps/mc/glyph.html>)をWorkerスレッドとOffscreenCanvasを用いてリファクタリングした。
流れでゲームMinecraft用の計算アプリslime_chunk_be(<https://mcbeeringi.github.io/apps/mc/slime_.html>)を作り直すことにした。
slime_chunk_beはその用途の性質上glyph_genと比較して高速に画像を生成する必要がある。
古いブラウザの場合OffscreenCanvasに対応していないものがある。
しかしWorkerスレッドでOffscreenCanvasが使えない場合に同様の処理をメインスレッドで実行してしまうと、描画をブロックしてしまいブラウザが操作を受け付けない状態になる。
簡易な画像をWorkerスレッド上で高速に描画してメインスレッドでデコードしやすい状態で転送できる仕組みを作る必要が出てきた。

## 要件

座標を与えるとそれがスライムチャンクであるか否かを`Boolean`で返す関数`slime`がある。
`slime`は内部で簡易的なメルセンヌツイスタを実装しており、比較的重い処理である。
半透明の灰色を背景に`slime(p)==true`の座標を緑色で示した32*32ピクセルの画像を得たい。
形状の認識をしやすくするため薄く市松模様をかける。
これらは全てWorkerスレッドの中で行われる。

## 目標

- 📱 iOS12のWorkerスレッドで動く
- 💫 ArrayBuffer不使用
- 🔄️ 同期処理
- ⚡ 爆速 (標準APIもびっくり!)

## 手法

### 既存

計算結果をRGBAに変換したデータを転送して全ての描画をメインスレッドで行う手法である。

- Workerスレッドで`slime`計算、色を決定してRGBAの順で配列に格納
- `Uint8ClampedArray`に変換しメインスレッドに転送
- `ImageData`に変換
- canvasを用意して`putImageData`で描画
- `URL.createObjectURL(await canvas.toBlob())`でURLを取得
- img要素にデコード
- 画面に表示されるcanvasに描画

この手法には多数のデメリットが存在する。

- TypedArrayは通常の配列に比べてパフォーマンスが劣る
- canvasを毎度新規で用意する行為はパフォーマンスを大きく低下させる要因
- 数色しか使用しないにもかかわらずRGBAの状態で転送する行為は情報密度が低く好ましくない

### 今回

計算結果を可能な限り画像に近づけた状態で転送して最低限の描画をメインスレッドで行う手法である。

- 画像のバイナリを構成する定数やパレットをあらかじめ用意
- Workerスレッドで`slime`計算、色を決定してインデックス番号として配列に格納
- Base64エンコードしてDataURLを成形、メインスレッドに転送
- img要素にデコード
- 画面に表示されるcanvasに描画

既存の手法との比較は以下の通り。

- メインスレッドでデコードのcanvasを用いない
- canvasからのURL取得と異なり画像圧縮を挟まないので高速
- 色をインデックス番号で管理可能なため高効率

## フォーマットの選定

画像圧縮を省くことで高速に処理をしたいので、非圧縮の画像フォーマットについて調べた。

- PNM
  - テキスト形式とバイナリ形式がある
  - tips: 弊学の生徒は一年次の授業でPNM画像をRubyで生成する
- BMP
  - ブラウザで使用可能
  - インデックスカラーが使用可能、つまりパレットの定義が可能

BMPを試すことにした。

## BMP

主題ではないので解説は軽く済ませる。

### 構造

Wikipediaが参考になる。
とても古いフォーマットであるが故に方言が存在する様である。
今回はWindowsのv4ヘッダを用いた。
以下Microsoftからの引用である。

```c
typedef struct {
  DWORD        bV4Size;
  LONG         bV4Width;
  LONG         bV4Height;
  WORD         bV4Planes;
  WORD         bV4BitCount;
  DWORD        bV4V4Compression;
  DWORD        bV4SizeImage;
  LONG         bV4XPelsPerMeter;
  LONG         bV4YPelsPerMeter;
  DWORD        bV4ClrUsed;
  DWORD        bV4ClrImportant;
  DWORD        bV4RedMask;
  DWORD        bV4GreenMask;
  DWORD        bV4BlueMask;
  DWORD        bV4AlphaMask;
  DWORD        bV4CSType;
  CIEXYZTRIPLE bV4Endpoints;
  DWORD        bV4GammaRed;
  DWORD        bV4GammaGreen;
  DWORD        bV4GammaBlue;
} BITMAPV4HEADER, *LPBITMAPV4HEADER, *PBITMAPV4HEADER;
```

### 実装

以下が要件を満たすBMPを返すJSの実装である。
この後述べる問題点の影響で途中までの実装となっており、インデックスカラーは用いていない。
理論上不要な項目が設定されていたり色順が理不尽であったりするのは、こうでなければ動作しなかったためである。
<https://github.com/McbeEringi/mcbeeringi.github.io/blob/e68088fec691cffdfe94a03682636ced39ef5031/apps/mc/slime_.html>

```js
const wmain=p=>'data:image/bmp;base64,'+btoa(String.fromCharCode(// little endian
  // file
  66,77,
  122,16,0,0,// file size: 14+108+4*1024=4218
  0,0,0,0,
  122,0,0,0,// offset: 14+108=122
  // info v4
  108,0,0,0,
  32,0,0,0,// width: 32
  32,0,0,0,// height: 32
  1,0,
  32,0,// bit/px: 32
  3,0,0,0,// compress: RGB
  0,16,0,0,// img size: 32*32*32/8=4096
  19,11,0,0,// x px/m: 72 dpi
  19,11,0,0,// y px/m: 72 dpi
  0,0,0,0,// col cnt
  0,0,0,0,// col imp cont
  0,0,255,0, 0,255,0,0, 255,0,0,0, 0,0,0,255,// mask RGBA: ARGB
  66,71,82,115,...Array(48).fill(0),// color: sRGB
  // img Y-flipped BGRA
  ...[...Array(1024)].flatMap((_,i,{length:l})=>(l=Math.sqrt(l),i=[p[0]*l+i%l,p[1]*l+(l-1)-(i/l|0)],[
    [128,128,128,32],[128,128,128,64],[0,255,0,224],[0,255,0,192],
    [128,128,128,128],[128,128,128,64],[0,255,0,224],[0,255,0,240]
  ][((p[0]^p[1])&1)|(slime(i)<<1)|(((i[0]^i[1])&1)<<2)]))
));
```

### 問題点

iPadのSafariで実行したところ何も表示されない。
言うまでもない、SafariはBMPをサポートしていなかったのである。

## PNG

### 非圧縮PNG

PNGは可逆圧縮を用いた画像ファイルフォーマットであり、今日において最も普及している画像フォーマットの一つである。
今回は非圧縮のフォーマットに焦点を当てているにもかかわらずなぜPNGが登場するのか。
それはPNGの仕様上圧縮をせずともPNG画像を作成することが可能であるからである。

PNGはそのデータの圧縮にzlibという形式を用いている。
zlibはdeflate圧縮アルゴリズムを実装したライブラリであり、同時にヘッダやフッタの定義も含んでいる。
deflateは広く用いられているデータ圧縮アルゴリズムである。
zipファイルやgzipといった広く用いられる技術の裏側で活躍している。

deflateはデータをある一定サイズ以下のチャンクの並びとして保持する。
チャンクには3種類あり、それぞれダイナミックハフマンブロック、固定ハフマンブロック、非圧縮ブロックである。
このうち非圧縮ブロックを用いることで、deflate、zlib、PNGの仕様を満たした状態で非圧縮のPNG画像が生成可能なのである。

### 構造

- 基本的にビッグエンディアン
- ファイル構造
  - ファイルヘッダ
  - チャンク
  - チャンク
  - ...
  - チャンク

### ファイルヘッダ

8byteの固定のデータである。
PNGファイルであることの確認とファイルの破損の検出のためのもの。

```text
89 50 4e 47 0d 0a 1a 0a
```

### チャンク

ファイルヘッダ以外のすべてのデータはチャンクの中に存在する
チャンクは数種類あるが構造は共通である。

- length ( 4 byte )
  - dataの長さをビッグエンディアンで
- chunk type ( 4 byte )
  - IHDR, IDATなどのチャンク名
- data ( n byte )
  - 任意のデータ
- CRC32 ( 4 byte )
  - データの破損を検出するためのデータ
  - chunk type + dataのCRC32をビッグエンディアンで
  - zipファイルの生成に全く同じものを使ったのでこれを流用した(ZIP編を参照)

代表的なチャンクを以下に挙げる。
幾つかは設置が必須のチャンク( * )であり必ず含める必要がある。

#### IHDR *

設置が必須のチャンクである。
画像の基本的な情報を格納する。

- Width ( 4 bytes )
  - 画像の幅
- Height ( 4 bytes )
  - 画像の高さ
- Bit depth ( 1 byte )
  - 1ピクセルを表すのに用いるビット数を表す
- Color type ( 1 byte )
  - 色の扱い方
- Compression method ( 1 byte )
  - 常に0
- Filter method ( 1 byte )
  - 常に0
- Interlace method ( 1 byte )
  - インターレス(Adam7)を使用する(1)か否(0)か

Color typeとBit depthは使用できる値とその組み合わせが決まっている。
| Color type | Bit depth |機能|説明|
|:-:|:-:|:-:|:-:|
| 0 | 1,2,4,8,16 | グレースケール|ピクセルの明るさを{Bit depth}bitで表現 |
| 2 | 8,16 | TrueColor|ピクセルの色(RGB)を{Bit depth}*3byteで表現 |
| 3 | 1,2,4,8 | インデックス|パレット(PLTEチャンク)で定義した色を参照 |
| 4 | 8,16 | アルファ付き グレースケール | ピクセルの明るさと透明度(GA)を{Bit depth}*2bitで表現 |
| 6 | 8,16 | アルファ付き TrueColor |ピクセルの色と透明度(RGBA)を{Bit depth}*4byteで表現 |

```text
00 00 00 0d 49 48 44 52 0 0 0 20 0 0 0 20 2 3 0 0 0 0e 14 92 67
length----- chunk_type- width--- height-- - - - - - CRC32------
13          I  H  D  R  32       32       2bit indexed
```

#### PLTE

インデックスモードで用いる色をRGBで列挙する。
順番がそのままインデックス番号になる。

```text
00 00 00 03 50 4c 54 45 ff 00 ff 34 e0 e6 ba
length----- chunk_type- data---- CRC32------
3           P  L  T  E  #ff00ff
{0:#ff00ff}なパレットの定義
```

#### tRNS

透明度を指定したい場合に使用する。
グレースケールモードとTrueColorモードにおいてはここで指定した色が透明になる。
インデックスモードにおいては透明度を列挙することで各インデックス番号における透明度を指定できる。
透明度をIDATに書けるモードにおいては使用できない。

```text
00 00 00 01 74 52 4e 53 80 ad 5e 5b 46
length----- chunk_type- da CRC32------
1           t  R  N  S  
インデックス番号0の色の透明度を0x80に設定
```

#### IDAT *

設置が必須のチャンクである。
zlibで圧縮した画像データを持つチャンクである。
一つのファイルに複数個のIDATチャンクを含めることができる。
画像データについては後ほど解説する。

```text
00 00 00 06 49 44 41 54 00 01 23 00 45 67 e1 c9 7f 1b
length----- chunk_type- data------------- CRC32------
6           I  D  A  T
```

#### IEND *

設置が必須のチャンクである。
ファイルの終端であることを示すチャンク。
これ以降のデータは読み込まれない。
データは常に空でありどのファイルでも以下の通りである。

```text
00 00 00 00 49 45 4e 44 ae 42 60 82
length----- chunk_type- CRC32------
0           I  E  N  D
```

### zlib

zlibはdeflate圧縮アルゴリズムを用いたライブラリ + ヘッダとフッタの定義である。
吐かれるデータはヘッダ+deflateのブロックたち+フッタである。

#### ヘッダ

- CMF ( 1 byte )
  - `CMF=(CINFO&15)<<4|(CM&15)`
  - CINFO ( 4 bits )
    - 圧縮情報
    - deflateで使った窓のサイズを0~7であらわす
  - CM ( 4 bits )
    - 圧縮方式
    - 8がdeflateを表す
- FLG ( 1 byte )
  - `FLG=(FLEVEL&3)<<6|(FDICT&1)<<5|(FCHECK&31)`
  - FLEVEL ( 2 bits )
    - 圧縮に使ったアルゴリズムについて
    - 圧縮速度が`速 0~3 遅`
    - 圧縮結果が`大 0~3 小`
  - FDICT ( 1 bit )
    - プリセット辞書を使う(1)か否(0)か
  - FCHECK ( 5 bits )
    - データ破損チェック用
    - `(CMF<<8|FLG)%31==0`になるような数を入れる
- DICTID ( 4 bytes )
  - `FDICT==1`の場合に出現

今回は非圧縮ブロックを使うのでヘッダは`08 1d`になる

#### deflate

圧縮したデータはPNGのチャンクと同じようにブロックという単位に区切られる。
ブロックには数種類あり生のデータをそのまま入れられる非圧縮ブロックが存在する。

##### 非圧縮ブロック

- flag ( 1 byte )
  - `flag=(BTYPE&3)<<1|(BFINAL&1)`
  - BTYPE ( 2 bits )
    - 非圧縮ブロックは0
  - BFINAL ( 1bit )
    - このブロックが最後のブロックの場合に1
- len  ( 2 bytes )
  - dataの長さを**リトルエンディアン**で
- nlen ( 2 bytes )
  - lenのビット反転
- data ( n bytes )
  - ここにデータが入る

以下は`01 02 03`を2つの非圧縮ブロックに分けて入れる例である。

```text
00 01 00 fe ff 01
01 02 00 fd ff 02 03
  len-- nlen- data-
```

#### adler32 (フッタ)

役割も長さもCRC32と同じである。
圧縮前のデータから計算した結果をビッグエンディアンで格納する。
wikipediaを参考に実装した。

```js
adler=data=>{
    let a=1,b=0,len=data.length,tlen,i=0;
    while(len>0){
        len-=(tlen=Math.min(1024,len));
        do{b+=(a+=data[i++]);}while(--tlen);
        a%=65521;b%=65521;
    }
    return(b<<16)|a;
}
```

### PNG画像データ

deflateにかける前のデータである。
圧縮効率を高めるため行毎にフィルターを定義できる。

<https://www.w3.org/TR/PNG-Filters.html>

0が何もしていない生のデータを表す。
フィルターの種類を表す数は行の先頭に付加する。
データはIHDRで定義した通りに書く。
例えば1pxあたり8bitでインデックス番号が

```text
0123
4567
````

の画像のデータはフィルター0を使用すると

```text
0 0 1 2 3 0 4 5 6 7
- ------- - -------
```

になる。

### 実装

以下が要件を満たす非圧縮PNGを返すJSの実装である。
<https://github.com/McbeEringi/mcbeeringi.github.io/blob/b4d608a17eb04bc752fe3801ed2aaf2b89fdf3b2/apps/mc/slime_.html>

```js
const main=p=>(_=>(
  _=[...Array(256)].flatMap((_,i,{length:m})=>(_=[...Array(4)].reduce((a,_,j,{length:n})=>(
    _={i:i*n+j,l:Math.sqrt(m*n)},_=[p[0]*_.l+_.i%_.l,p[1]*_.l+(_.i/_.l|0)],
    (slime(_)<<1|(_[0]^_[1])&1)<<(8/n*(n-1-j))|a
  ),0),i%8?[_]:[0,_])),
  _=[73,68,65,84, 8,29, 1, 32,1, 223,254, ..._,...be4(adler(_))],// IDAT zlib CMF:08 FLG:1d uncompressed final block length:256+32=288(little endian)
  'data:image/png;base64,'+btoa(String.fromCharCode(// big endian
    137,80,78,71,13,10,26,10,// header
    0,0,0,13,73,72,68,82, 0,0,0,32,0,0,0,32, 2,3, 0,0,0, 14,20,146,103,// IHDR: 32*32 2bit indexed
    0,0,0,12,80,76,84,69, 128,128,128, 128,128,128, 0,255,0, 0,255,0, 32,242,49,113,// PLTE
    0,0,0,4,116,82,78,83, ...((p[0]^p[1])&1?[128,64,224,240, 205,147,92,145]:[32,64,224,192, 166,33,117,56]),// tRNS
    0,0,1,43,..._,...be4(crc(_)),// IDAT
    0,0,0,0,73,69,78,68,174,66,96,130// IEND
  ))
))();
```

## 検証

canvasとOffscreenCanvasと今回のPNG生成をメインスレッドで1000回実行して比較した。
単純な画像エンコーダとしての性能を見るため、32*32のノイズを用いた。
OffscreenCanvasにはtoDataURLが存在しないのでtoBlobで代用した。

```js
const
ctx=Object.assign(document.createElement('canvas'),{width:32,height:32}).getContext('2d'),
octx=new OffscreenCanvas(32,32).getContext('2d'),

plte=[[0,255,255,255],[255,0,255,255],[255,255,0,255],[0,0,0,255]],
draw=()=>new Promise(f=>Object.assign(new Image(),{
    onload(){f(this);},
    src:((
        // CRC32
        crct=[...Array(256)].map((_,n)=>[...Array(8)].reduce(c=>(c&1)?0xedb88320^(c>>>1):c>>>1,n)),
        crc=(buf,crc=0)=>~buf.reduce((c,x)=>crct[(c^x)&0xff]^(c>>>8),~crc),
        // adler32
        adler=data=>{let a=1,b=0,len=data.length,tlen,i=0;while(len>0){len-=(tlen=Math.min(1024,len));do{b+=(a+=data[i++]);}while(--tlen);a%=65521;b%=65521;}return(b<<16)|a;},
        // ヘルパー
        be4=x=>[x>>>24&255,x>>>16&255,x>>>8&255,x>>>0&255],// 32bit -> ビッグエンディアン4bytes
        arr=x=>x.match(/[\da-f]{2}/g).map(x=>parseInt(x,16)),// hex文字列 -> 配列
        chunk=x=>[...be4(x.length-4),...x,...be4(crc(x))],// データ -> チャンク
        
        // 2bit*32*32のPNG画像データを用意
        img=[...Array(32)].flatMap(_=>[
            0,// フィルター
            ...[...Array(32/(8/2))].map(_=>Math.random()*255|0)// ランダムに
        ])
    )=>'data:image/png;base64,'+btoa(String.fromCharCode(
        ...arr('89 50 4e 47 0d 0a 1a 0a'),// PNGヘッダ
        ...chunk(arr('49 48 44 52  00 00 00 20 00 00 00 20  02 03 00 00 00')),// IHDR 32*32 2bit indexed
        ...chunk(arr('50 4c 54 45  00 ff ff  ff 00 ff  ff ff 00  00 00 00')),// PLTE CMYK
        ...chunk([// IDAT "IDAT" + zlib * 非圧縮ブロック
            0x49,0x44,0x41,0x54,
            0x08,0x1d, 1, ...(_=>[_>>>0&255,_>>>8&255,~_>>>0&255,~_>>>8&255])(img.length),
            ...img, ...be4(adler(img))
        ]),
        ...chunk(arr('49 45 4e 44'))// IEND
    )))()
})),
cdraw=(
  img=[...Array(32)].flatMap(_=>[...Array(32)].flatMap(_=>plte[Math.random()*4|0]))
)=>new Promise(f=>Object.assign(new Image(),{
  onload(){f(this);},
  src:(
    ctx.putImageData(new ImageData(new Uint8ClampedArray(img),32),0,0),
    ctx.canvas.toDataURL()
  )
})),
ocdraw=(
  img=[...Array(32)].flatMap(_=>[...Array(32)].flatMap(_=>plte[Math.random()*4|0])),
  url
)=>new Promise(async f=>Object.assign(new Image(),{
  onload(){URL.revokeObjectURL(url);f(this);},
  src:(
    octx.putImageData(new ImageData(new Uint8ClampedArray(img),32),0,0),
    url=URL.createObjectURL(await octx.canvas.convertToBlob())
  )
}));

((
  t0=performance.now(),
  log=x=>console.log(x,-t0+(t0=performance.now()))
)=>(
  await  cdraw(),log('canvas'),
  await ocdraw(),log('OffscreenCanvas'),
  await   draw(),log('png asm')
))();

```

CPU: N100, RAM: 16GB
Windows11 Firefox 117.0.1 64bit

の環境で上記コードを実行した。

- 2059 ms (canvas)
- 788 ms (OffscreenCanvas)
- 389 ms (今回の手法)

dataurlとObjectURLの差を確認するため全てObjectURLを使った場合も測定した。

- 1939 ms (canvas)
- 745 ms (OffscreenCanvas)
- 522 ms (今回の手法)

## 結論

今回のPNG生成手法はcanvasに対して約5倍、OffscreenCanvasに対して約1.5倍高速である問結果が得られた。
目標は十分に達成された。

- ✅ iOS12のWorkerスレッドで動く
- ✅ ArrayBuffer不使用
- ✅ 同期処理
- ✅ 爆速 (ぶっちぎり)

また、deflateにおいて圧縮は必須ではないこと、OffscreenCanvasはcanvasとは全くの別物であることなどの知見が得られた。
時間があればAPNGの生成にも挑戦したい。

## 参考文献

<https://ja.wikipedia.org/wiki/Windows_bitmap>

<https://learn.microsoft.com/en-us/windows/win32/api/wingdi/ns-wingdi-bitmapv4header>

<https://www.w3.org/TR/png/>

<http://www.libpng.org/pub/png/apps/pngcheck.html>

<https://ja.wikipedia.org/wiki/Portable_Network_Graphics>

<https://ja.wikipedia.org/wiki/Adler-32>

<https://www.setsuki.com/hsp/ext/png.htm>

<https://zenn.dev/yohhoy/articles/exploding-pngfile>

<https://kimaguresoft.blogspot.com/2019/03/png1.html>

<https://darkcrowcorvus.hatenablog.jp/entry/2017/01/09/014407>

<https://datatracker.ietf.org/doc/html/rfc1950>

<https://datatracker.ietf.org/doc/html/rfc1951>

<https://github.com/imaya/zlib.js/blob/develop/src/deflate.js>
